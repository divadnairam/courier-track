...
export default async function ArhivaPage({
  ...
  if (session?.user.role === "CLIENT") {
    ...
    if (client) {
      ...
      return (
        <div>
          <PageHeader title="Arhivă Colete" description="0 colete în arhivă" />
          <div className="rounded-lg border bg-white p-8 text-center text-gray-500">
            Nu au fost găsite colete în arhivă
          </div>
        </div>
      );
    }
  ...
  return (
    <div>
      <PageHeader
        title="Arhivă Colete"
        description={`${total} colete livrate sau returnate`}
      />
      ...
      <div className="mb-4">
        <form className="flex gap-2 flex-wrap">
          ...
          <button
            type="submit"
            className="rounded-lg bg-primary px-4 py-2 text-sm text-primary-foreground hover:bg-primary/80"
          >
            Caută
          </button>
        </form>
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="hidden md:table-cell">Data finalizării</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {parcels.map((parcel) => (
              <TableRow key={parcel.id}>
                ...
                <TableCell className="text-xs">
                  {parcel.pickupCity}
                  {parcel.pickupCountry !== "RO"
                    ? `, ${COUNTRY_LABELS[parcel.pickupCountry] || parcel.pickupCountry}`
                    : ""}{" "}
                  → {parcel.deliveryCity}
                  {parcel.deliveryCountry !== "RO"
                    ? `, ${COUNTRY_LABELS[parcel.deliveryCountry] || parcel.deliveryCountry}`
                    : ""}
                </TableCell>
                <TableCell>
                ...
                <TableCell className="hidden md:table-cell">
                  {parcel.statusHistory[0]
                    ? formatDate(parcel.statusHistory[0].createdAt)
      ...
      {totalPages > 1 && (
        <div className="mt-4 flex justify-center gap-2">
          {Array.from({ length: totalPages }, (_, i) => i + 1).map((p) => (
            <Link
              key={p}
              href={`/dashboard/arhiva?search=${search}&page=${p}`}
